...
- **Optimal Assignment rule** $\pi^*=\arg\max_{\pi \in \Pi} Q(\pi)$, where  $Q(\pi) =  \mathbb{E} [Y^{\pi(V)}]$
...
- Algorithm that conducts an exhaustive and recursive search over the variables and their values. It splits when the welfare within the leaves is maximised by assigning all observations in a terminal leaf node to a single treatment
...
::: {.incremental}
- Unsupervised machine learning method that partitions data into distinct clusters by minimizing the variance within each cluster (Arthur and Vassilvitskii, 2007)
...
## Policy Trees {.center}
::: {.incremental}
- **Simulate programme assignments** to assess potential improvements with respect to observed and random allocations of unemployed
