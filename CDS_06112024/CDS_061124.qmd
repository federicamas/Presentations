...
- **BGATE** $\implies \tau_{d}^B(z) :=  \mathbb{E}\left[\mathbb{E}[ \,  Y^d - Y^{0} \, | \,  Z = z, W]\right]$ 
    - average treatment effect for a larger group of interest balanced for a subgorup of the covariates
...
- **Optimal Policy rule** $\pi^*=\arg\max_{\pi \in \Pi} Q(\pi)$, where  $Q(\pi) =  \mathbb{E} [Y^{\pi(V)}]$
...
- $Q(\pi)$ Value function, average potential outcome under the policy



## Indentification {.center}
...
- Package freely available on Github, MCFpy
...
::: {.incremental}
Difference BGATE - ATE of **Wage Subsidy** with respect to non-participants for earnings in the third year after the programme start for Permanent Residents sample 
...
for earnings and employment in the third year after the programmes start for Permanent Residents sample 
...
::: {.incremental}
- Group indivduals based on the size of their IATEs
- K-means clustering (Arthur and Vassilvitskii, 2007) is an unsupervised machine learning method that partitions data into distinct clusters by minimizing the variance within each cluster
:::

. . .

Clusters that show greatest benefit: 
...
- Individuals with **non-Swiss nationality** benefit more, from all the programmes
:::

<!---->

## Policy Learning {.center}
::: {.incremental}
- **Simulate programme assignments** to assess potential improvements, measuring gains in average months of employment 
- **Compare simulated assignments** to actual programme allocations and their corresponding average employment outcomes
- **Policy Tree**: algorithm conducts an exhaustive and recursive search over the policy variables and their values. It splits when the welfare within the leaves is maximised by assigning all observations in a terminal leaf node to a single treatment
:::
...
- **Heterogeneous Effects:**
    ...
    -->
    - Statistically insignificant results for the smaller sample of Temporary Residents

- **Simulated allocations:**
    - Increase assignments to WS for both samples
    - Use deeper policy trees in Permanent Residents, if the all the programmes need to be maintained 
